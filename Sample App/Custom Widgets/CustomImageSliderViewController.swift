...
class CustomImageSliderViewController: Widget {
    ...
    override func viewDidLoad() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + model.interactionTimeInterval) { [weak self] in
            ...
            self.model.lockInVote(magnitude: Double(self.imageSliderView.slider.value)) { result in
    ...
    func updateMagnitudeIndicator(_ magnitude: Double) {
        self.magnitudeIndicatorLeadingConstraint?.isActive = false

        let averageXPosition = CGFloat(magnitude) * self.imageSliderView.slider.bounds.width
        self.magnitudeIndicatorLeadingConstraint = self.magnitudeIndicator.centerXAnchor.constraint(
            equalTo: self.imageSliderView.slider.leadingAnchor,
            constant: averageXPosition
        )
        self.magnitudeIndicatorLeadingConstraint?.isActive = true
    }
