...
class ImagePredictionWidgetResultsViewController: Widget, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "myCell", for: indexPath) as? CustomImageOptionCell {
            ...
            let progress = model.totalVoteCount > 0 ? CGFloat(option.voteCount) / CGFloat(model.totalVoteCount) : 0
