...
class FrameStore {
    ...
    private lazy var preloadFrameQueue: DispatchQueue = {
        DispatchQueue(label: "co.kaishin.Gifu.preloadQueue")
    }()
    ...
    init(data: Data, size: CGSize, contentMode: UIView.ContentMode, framePreloadCount: Int, loopCount: Int) {
        ...
        imageSource = CGImageSourceCreateWithData(data as CFData, options) ?? CGImageSourceCreateIncremental(options)
        self.size = size
        self.contentMode = contentMode
        bufferFrameCount = framePreloadCount
